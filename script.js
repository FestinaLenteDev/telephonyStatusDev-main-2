...
async function fetchFromBackend(endpoint) {
    try {
        // Construct the full URL using the Azure API base URL
        const response = await fetch(`${apiBaseUrl}/${endpoint}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json', // Specify the expected response type
            },
        });
        ...
        console.log(`Data from backend (${endpoint}):`, data);
...
async function fetchStatuses() {
    try {
        const fetchedData = await fetchFromBackend('statuses');
...
async function fetchHistoricStatuses() {
    try {
        const historicData = await fetchFromBackend('status_history');

        if (!historicData || historicData.length === 0) {
            console.error('Error fetching historic statuses: No data found');
        ...
        console.error('Unexpected error while fetching historic data:', error);
